...
app = FastAPI(title="FinAdvisor API", version="5.0")
...
async def root():
    return {
        "status": "ok",
        "message": "FinAdvisor API v5.0 — Powered by Groq (LLaMA3)",
        "ai_ready": bool(GROQ_API_KEY and GROQ_AVAILABLE),
    }
...
async def health():
    return {
        ...
        "groq_configured": bool(GROQ_API_KEY and GROQ_AVAILABLE),
...
async def parse_form16(file: UploadFile = File(...)):
    ...
    if not GROQ_API_KEY or not GROQ_AVAILABLE:
        return {"status": "success", "data": fallback_data, "note": "GROQ_API_KEY not set"}

    try:
        client = Groq(api_key=GROQ_API_KEY)

        prompt = f"""Extract financial data from this Indian Form 16 document.
Return ONLY a valid JSON object with these exact keys (numeric values only, no commas, no currency symbols):
{{
  "gross_salary": <number>,
  "tax_deducted": <number>,
...
async def chat(req: ChatRequest):
    ...
    if not GROQ_API_KEY or not GROQ_AVAILABLE:
        raise HTTPException(status_code=503, detail="GROQ_API_KEY not set in Render Environment")

    try:
        client = Groq(api_key=GROQ_API_KEY)

        system_prompt = """You are FinBot, an expert Indian financial advisor.
...
Respond in the same language the user writes in (Hindi or English)."""
        ...
        if req.user_data:
            user_msg = f"My financial profile: {json.dumps(req.user_data)}\n\nQuestion: {req.message}"
        ...
        if "api_key" in err.lower() or "auth" in err.lower() or "401" in err:
            raise HTTPException(status_code=401, detail="Invalid GROQ_API_KEY — check Render environment variables")
...
async def generate_report(data: ReportRequest):
    ...
    try:
        ...
        table_data = [
            ...
            ["Savings Rate",
             f"{(data.monthly_savings / (data.gross_salary / 12) * 100):.1f}%"
        ...
        story.append(Paragraph(
            "<i>Disclaimer: For informational purposes only. "
            "Consult a SEBI-registered advisor.</i>",
            styles["Normal"]
        ))
        doc.build(story)
